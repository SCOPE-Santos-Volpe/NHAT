...
class Bin:
    ...
    Attributes:
        ...
        bin_points: a `shapely.geometry.linestring.LineString` with points that define this bin. More granular than shape_points
...
def get_sds_crashes(table_name, county_id, start_year, from_crs):
    ...
    sds_query = db.select([SDS_table]).where(db.and_(SDS_table.columns.COUNTY_ID ==
                                                     county_id, SDS_table.columns.YEAR > start_year, SDS_table.columns.SEVERITY > 0))
...
def generate_hin(state_id, county_id):
    try:
...
if __name__=="__main__":
    ...
    for state in state_ids:
        for county in county_ids:
            params.append((state,county))

    with multiprocessing.Pool() as pool:
        pool.starmap(generate_hin, params)