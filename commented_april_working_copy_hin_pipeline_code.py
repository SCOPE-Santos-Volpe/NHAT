...
def get_county_boundaries_from_rds(state_id: int, county_id: int = None, mpo_id: int = None) -> gpd.GeoDataFrame:
    ...
    if county_id is not None:
        sql = text(""" SELECT * FROM "boundaries_county" WHERE "STATE_ID" = {} AND "COUNTY_ID" = '{}' """.format(state_id, county_id))
    elif mpo_id is not None:
        sql = text(""" SELECT * FROM "boundaries_county" WHERE "STATE_ID" = {} AND "MPO_ID" = '{}' """.format(state_id, mpo_id))
...
def get_graph_from_county(county_bounds: gpd.GeoDataFrame):
    ...
    G = ox.graph.graph_from_polygon(combined_multipolygon,network_type='drive') 
...
class Bin:
  def __init__(self, ids, shape_points, bin_points, times_counted):
    ...
    self.times_counted = times_counted # number of times this bin will get counted by sliding windows
    self.multiplier = 5/times_counted # multiplier to correct for undercounting
    self.crash_points = []
    self.crash_weights = []

  def make_buffer(self, radius):
    self.buffer = self.shape_points.buffer(radius, cap_style=2) # square off ends of buffer
    return self.buffer
  ...
  def get_crashes(self):
    self.multiplied_weights = self.multiplier * np.array(self.crash_weights) 
...
class Window:
  ...
  def get_center(self):
    ...
    center_point = self.shape_points.interpolate(self.shape_points.length/2)
    return center_point
...
class Corridor:
  def __init__(self, ids, shape_points, corridor_points, bins, windows):
    ...
    self.shape_points = shape_points # points that define the shape of this road corridor
  ...
  def run_kde(self, bandwidth):
    ...
    if sum([i > 0 for i in self.crash_weights]) >= 1: # Case 1: can run KDE
      ...
      if len(self.modified_crash_weights) == 1: # Edge case: need to distribute crash weight in order to make KDE work
        # TODO: think about whether spreading out crash weight like this is valid
        self.modified_crash_weights = [self.modified_crash_weights[0]*0.1,self.modified_crash_weights[0]*0.8,self.modified_crash_weights[0]*0.1] 
        self.modified_center_points = [Point(self.windows[0].window_points.coords[0]), self.modified_center_points[0], Point(self.windows[0].window_points.coords[-1])]
        
        center_points_1d = [0, self.windows[0].shape_points.length/2, self.windows[0].shape_points.length]

      elif sum([i > 0 for i in self.modified_crash_weights]) == 1: # Edge case: need to distribute crash weights in order to run KDE
        if self.modified_crash_weights[0] > 0:
          ...
          self.modified_crash_weights[idx+1] = self.modified_crash_weights[idx]*0.1
          self.modified_crash_weights[idx-1] = self.modified_crash_weights[idx]*0.1
      ...
      kde = st.gaussian_kde(center_points_1d, weights=self.modified_crash_weights, bw_method=bandwidth) # based on experiments, bandwidth 0.24 worked fairly well

      x = np.linspace(0,self.shape_points.length,num=100) # makes 100 points along road length
...
def create_bins(edges):
    ...
    for edge in edges.iterfeatures():
        ...
        distances_granular = np.arange(0, line.length, distance_delta/scale_factor)

        if not line.boundary.is_empty:
            points = [line.interpolate(distance) for distance in distances] + [line.boundary.geoms[1]]
            points_granular = [line.interpolate(distance) for distance in distances_granular] + [line.boundary.geoms[1]]
        else:
            points = [line.interpolate(distance) for distance in distances] + [edge["geometry"]["coordinates"][0]]
            points_granular = [line.interpolate(distance) for distance in distances_granular] + [edge["geometry"]["coordinates"][0]]
        ...
        for i in range(len(points)-1):
            if len(points) > 1:
                ...
                bin_granular_linestring = LineString(points_granular[i*scale_factor:(i+1)*scale_factor+1])
            ...
            bin = Bin(ids, bin_granular_linestring, bin_linestring, times_counted)
...
def create_windows(edges, bins_by_edge, points_by_edge, points_granular_by_edge):
    ...
    for edge in edges.iterfeatures():
        ...
        for j in range(num_windows):
            ...
            linestring = LineString(points_by_edge[tuple(ids)].coords[j:j+bins_per_window+1])
            linestring_granular = LineString(points_granular_by_edge[tuple(ids)].coords[j*scale_factor:(j+bins_per_window)*scale_factor+1])
...
def create_corridors(edges, points_granular_by_edge, points_by_edge, bins_by_edge, windows_by_edge):
    ...
    for edge in edges.iterfeatures():
        ids = edge["id"][1:-1].split()
        corridor = Corridor(ids, points_granular_by_edge[tuple(ids)], points_by_edge[tuple(ids)], bins_by_edge[tuple(ids)], windows_by_edge[tuple(ids)])
...
def get_fars_crashes(state_id, county_id, start_year):
  ...
  fars_query = db.select([FARS]).where(db.and_(FARS.columns.STATE_ID == state_id, FARS.columns.COUNTY_ID == county_id, FARS.columns.YEAR >= start_year))
  ...
  proj = pyproj.Transformer.from_crs(pyproj.CRS('EPSG:4326'), pyproj.CRS('EPSG:2805'), always_xy=True)
  x2, y2 = proj.transform(fars_pdf['LON'], fars_pdf['LAT'])
  fars_df_nsf = pd.DataFrame({'LON': x2, 'LAT': y2, 'espg': pyproj.CRS('EPSG:2805'), 'SEVERITY':fars_pdf['SEVERITY']}, index=fars_pdf['LAT'].index)

  # Turn LAT, LON into Point():
  fars_gdf = gpd.GeoDataFrame(fars_df_nsf, geometry=gpd.points_from_xy(fars_df_nsf['LON'], fars_df_nsf['LAT']), crs="EPSG:2805")
...
def get_sds_crashes(table_name, county_id, start_year, from_crs):
  ...
  SDS_table = db.Table(table_name, metadata, autoload=True, autoload_with=engine)

  sds_query = db.select([SDS_table]).where(db.and_(SDS_table.columns.COUNTY_ID == county_id, SDS_table.columns.YEAR > 2015, SDS_table.columns.SEVERITY > 0))
  ...
  # Transform into national survey feet from lat/lon:
  proj = pyproj.Transformer.from_crs(pyproj.CRS(from_crs), pyproj.CRS('EPSG:2805'), always_xy=True)
  x2, y2 = proj.transform(sds_pdf['LON'], sds_pdf['LAT'])
  sds_df_nsf = pd.DataFrame({'LON': x2, 'LAT': y2, 'espg': pyproj.CRS('EPSG:2805'), 'SEVERITY':sds_pdf['SEVERITY']}, index=sds_pdf['LAT'].index)

  # Turn LAT, LON into Point():
  sds_gdf = gpd.GeoDataFrame(sds_df_nsf, geometry=gpd.points_from_xy(sds_df_nsf['LON'], sds_df_nsf['LAT']), crs="EPSG:2805")
...
def get_nearest_edges_to_crashes(df1):
  ...
  [nearest_edge_ids,returned_dist] = ox.distance.nearest_edges(graph_proj, crash_xs, crash_ys, return_dist=True)
  nearest_edge_ids_tuple = [tuple([str(ne[0])+",", str(ne[1])+",", str(ne[2])]) for ne in nearest_edge_ids]
...
def move_crashes_to_edges(data_df, edges_by_id_tuple, nearest_edge_ids_tuple):
  ...
  for index, crash in df1.iterrows():
    ...
    nearest_point = nearest_edge_line.interpolate(nearest_edge_line.project(crash[0]))
    ...
    df1['corrected_geometry'][index] = Point(nearest_point.x,nearest_point.y) 
...
def calculate_unthresholded_hin(bandwidth, corridors_by_edge):
  ...
  projector = pyproj.Transformer.from_crs(pyproj.CRS('EPSG:2805'), pyproj.CRS('EPSG:4326'), always_xy=True).transform
  ...
  for idx in corridors_by_edge:
    ...
    if len(corr.crash_weights) > 0:
      ...
      if hin_points != None:
        ...
        hin_points_by_corridor = [[hin_points_latlon[j].x, hin_points_latlon[j].y] for j in range(len(hin_points_latlon))]
          ...
          if y[i] > thr_set:
            ...
            if len(weights_buffer) > 0:
              ...
              if len(weights_buffer) == 1: # In case this edge case becomes a problem
                original_edge_geometry_line = original_points_by_edge[tuple(corr.ids)]
                length_buffer = original_edge_geometry_line.length/(num_points_along_edge-1)
                ...
                weights_buffer = [weights_buffer[0], weights_buffer[0], weights_buffer[0]]
              ...
              for crash_pt in corr_crash_points:
                ...
                points_buffer_shape = points_buffer_linestring.buffer(6, cap_style=2) # square off ends of buffer
              ...
              num_crashes_by_thr[thr_set] = num_crashes_by_thr[thr_set] + num_crashes_on_thresholded_corridor
              num_crashes_on_thresholded_corridor = 0 # Clear out crashes after they are added to not double-count
              ...
              ftr = geojson.Feature(properties={"type":"polyline"}, weight=weights_buffer, geometry=LineString(points_buffer_latlon))
              features_by_thr[thr_set].append(ftr)
              length_by_thr[thr_set] += length_buffer # purely keeps track of total length, not broken down by corridor
              length_buffer = 0
...
def calculate_hin_statistics(df1, edges, road_length_unthr, length_by_thr, num_crashes_by_thr, threshold_settings):
    ...
    results['percent_road_length'][0.0] = road_length_unthr/total_road_length*100
...
def get_census_tract_boundaries_from_rds(state_id: int, county_id: int = None, mpo_id: int = None):
    ...
    from sqlalchemy import text
    if county_id is not None:
        sql = text(""" SELECT * FROM "boundaries_census_tract_v3" WHERE "STATE_ID" = {} AND "COUNTY_ID" = '{}' """.format(state_id, county_id))
    elif mpo_id is not None:
        sql = text(""" SELECT * FROM "boundaries_census_tract_v3" WHERE "STATE_ID" = {} AND "MPO_ID" = '{}' """.format(state_id, mpo_id))
    elif state_id is None:
...
def calculate_joined_features(state_id, county_id, features_by_thr, j40_bounds):
  ...
  for key in features_by_thr.keys():
    ...
    joined_gdf = gpd.sjoin(features_gdf, j40_bounds, how="inner", predicate="intersects")
    joined_gdf = joined_gdf.drop(columns=['index_right', 'STATE_ID', 'STATE_NAME', 'COUNTY_ID', 'COUNTY_NAME', 'MPO_NAME', 'CENSUS_TRACT_ID'])
    joined_gdf = joined_gdf.rename(columns ={'IDENTIFIED_AS_DISADVANTAGED':'IN_J40'})
...
sqlalchemy_conn, metadata, engine = preprocess_utils.connect_to_sqlalchemy(include_metadata=True,include_engine=True)
...
# HIN parameters:
bandwidth = 0.24
threshold_settings  = [0.002,0.001,0.0005] # TODO: choose better values based on std dev of y-distribution
...
for state_id in state_ids:
  for county_id in county_ids:
    ...
    bins_by_edge, original_points_by_edge, points_granular_by_edge, points_by_edge, edges_by_id_tuple = create_bins(edges)
    windows_by_edge = create_windows(edges, bins_by_edge, points_by_edge, points_granular_by_edge)
    corridors_by_edge = create_corridors(edges, points_granular_by_edge, points_by_edge, bins_by_edge, windows_by_edge)
    ...
    # Get Justice40 data:
    j40_bounds = get_census_tract_boundaries_from_rds(state_id, county_id=county_id)
    ...
    for idx in range(len(datasource)):
      ...
      df1 = move_crashes_to_edges(df1, edges_by_id_tuple, nearest_edge_ids_tuple)
      print("\tMoved crashes onto edges")

      # Bin crashes:
      clear_crashes_from_bins(bins_by_edge)
      put_crashes_into_bins(df1, bins_by_edge, edges_by_id_tuple, nearest_edge_ids_tuple)
      print("\tBinned crashes")

      # Calculate unthresholded HIN:
      road_length_unthr, features = calculate_unthresholded_hin(bandwidth, corridors_by_edge)
      print("\tGenerated HIN")

      # Calculate thresholds:
      features_by_thr,length_by_thr,num_crashes_by_thr = calculate_thresholds(state_id,county_id,corridors_by_edge,original_points_by_edge,threshold_settings)
      print("\tCalculated thresholds")

      # Calculate statistics:
      features_by_thr[0.0] = features
      results = calculate_hin_statistics(df1, edges, road_length_unthr, length_by_thr, num_crashes_by_thr, threshold_settings)
      print("\tCalculated statistics")

      # Run spatial join on J40 bounds:
      joined_features_by_thr = calculate_joined_features(state_id, county_id, features_by_thr, j40_bounds)
      print("\tAdded IN_J40 column")

      # Save to files and produce summary:
      summary = save_feature_collections(state_id, county_id, crash_data_source, threshold_settings, joined_features_by_thr, results)
      print(summary)
